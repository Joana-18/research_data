...
VOX_PATH = '/path/to/datasets/folder/VoxConverse/'
vox_TEST_PATH = os.path.join(VOX_PATH, 'Data/test')

vox_TEST_LABELS_PATH = os.path.join(VOX_PATH, 'GroundTruth/test')

vox_OUTPUT_PATH_EP = os.path.join(OUTPUT_PATH, 'VoxConverse/results/global/')
vox_OUTPUT_PATH_BASELINE = os.path.join(OUTPUT_PATH, 'VoxConverse/results/baseline/')
vox_OUTPUT_PATH_EXACT = os.path.join(OUTPUT_PATH, 'VoxConverse/results/exact/')
vox_ID = 2
...
print('-' * 20, "VOX GLOBAL TEST 2.1".center(5), '-' * 20, flush=True)
diarization(vox_ID, vox_TEST_PATH, vox_OUTPUT_PATH_EP + PY2_1 + 'test/', 
            min_speakers = 1, max_speakers = 21, version = v21, globalP=True)

print('-' * 20, "VOX GLOBAL TEST 3.1".center(5), '-' * 20, flush=True)
diarization(vox_ID, vox_TEST_PATH, vox_OUTPUT_PATH_EP + PY3_1 + 'test/',
             min_speakers = 1, max_speakers = 21, version = v31, globalP=True)


#---------------------------- BASELINE ----------------------------


print('-' * 20, "VOX BASELINE TEST 2.1".center(5), '-' * 20, flush=True)
diarization(vox_ID, vox_TEST_PATH, vox_OUTPUT_PATH_BASELINE + PY2_1 + 'test/', 
            None, None, v21, False)

print('-' * 20, "VOX BASELINE TEST 3.1".center(5), '-' * 20, flush=True)
diarization(vox_ID, vox_TEST_PATH, vox_OUTPUT_PATH_BASELINE + PY3_1 + 'test/', 
            None, None, version = v31)


#---------------------------- EXACT ----------------------------

print('-' * 20, "VOX EXACT NUM TEST 2.1".center(5), '-' * 20, flush=True)
diarization(vox_ID, vox_TEST_PATH, vox_OUTPUT_PATH_EXACT + PY2_1 + 'test/', 
            version = v21, exactNum = True, gt_path = vox_TEST_LABELS_PATH)

print('-' * 20, "VOX EXACT NUM TEST 3.1".center(5), '-' * 20, flush=True)
diarization(vox_ID, vox_TEST_PATH, vox_OUTPUT_PATH_EXACT + PY3_1 + 'test/', 
            version = v31, exactNum = True, gt_path = vox_TEST_LABELS_PATH)
