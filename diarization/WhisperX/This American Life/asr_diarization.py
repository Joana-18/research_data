...
TAL_DATASET_PATH = '/path/to/datasets/folder/This American Life - Test Set'
tal_TEST_PATH = os.path.join(TAL_DATASET_PATH, 'wav')

tal_TEST_LABELS_PATH = os.path.join(TAL_DATASET_PATH, 'rttm')

tal_OUTPUT_PATH_EP = os.path.join(OUTPUT_PATH, 'This American Life/results/global/')
tal_OUTPUT_PATH_BASELINE = os.path.join(OUTPUT_PATH, 'This American Life/results/baseline/')
tal_OUTPUT_PATH_EXACT = os.path.join(OUTPUT_PATH, 'This American Life/results/exact/')
tal_ID = 7
...
print('-' * 20, "TAL GLOBAL TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = tal_ID, 
            data_path = tal_TEST_PATH, 
            output_path = tal_OUTPUT_PATH_EP + PY2_1 + 'test/', 
            min_speakers = 3, 
            max_speakers = 41, 
            version = v21,
            globalP = True)

print('-' * 20, "TAL GLOBAL TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = tal_ID, 
            data_path = tal_TEST_PATH, 
            output_path = tal_OUTPUT_PATH_EP + PY3_1 + 'test/', 
            min_speakers = 3, 
            max_speakers = 41, 
            version = v31,
            globalP = True)
...
print('-' * 20, "TAL BASELINE TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = tal_ID, 
            data_path = tal_TEST_PATH, 
            output_path = tal_OUTPUT_PATH_BASELINE + PY2_1 + 'test/', 
            min_speakers = None,
            max_speakers = None,
            version = v21,
            globalP = False, exactNum = False)

print('-' * 20, "TAL BASELINE TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = tal_ID, 
            data_path = tal_TEST_PATH, 
            output_path = tal_OUTPUT_PATH_BASELINE + PY3_1 + 'test/', 
            min_speakers = None,
            max_speakers = None,
            version = v31,
            globalP = False, exactNum = False)
...
print('-' * 20, "TAL EXACT NUM TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = tal_ID, 
            data_path = tal_TEST_PATH, 
            output_path = tal_OUTPUT_PATH_EXACT + PY2_1 + 'test/', 
            version = v21, 
            exactNum = True, 
            gt_path = tal_TEST_LABELS_PATH)

print('-' * 20, "TAL EXACT NUM TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = tal_ID, 
            data_path = tal_TEST_PATH, 
            output_path = tal_OUTPUT_PATH_EXACT + PY3_1 + 'test/', 
            version = v31, 
            exactNum = True, 
            gt_path = tal_TEST_LABELS_PATH)