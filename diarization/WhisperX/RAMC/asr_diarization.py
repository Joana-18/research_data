...
RAMC_DATASET_PATH = '/path/to/datasets/folder/RAMC/MDT2021S003/'
ramc_TEST_PATH = os.path.join(RAMC_DATASET_PATH, 'WAV/test')

ramc_TEST_LABELS_PATH = os.path.join(RAMC_DATASET_PATH, 'rttm/test')

ramc_OUTPUT_PATH_EP = os.path.join(OUTPUT_PATH, 'RAMC/results/global/')
ramc_OUTPUT_PATH_BASELINE = os.path.join(OUTPUT_PATH, 'RAMC/results/baseline/')
ramc_OUTPUT_PATH_EXACT = os.path.join(OUTPUT_PATH, 'RAMC/results/exact/')
ramc_ID = 6
...
print('-' * 20, "RAMC GLOBAL TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = ramc_ID, 
            data_path = ramc_TEST_PATH, 
            output_path = ramc_OUTPUT_PATH_EP + PY2_1 + 'test/', 
            min_speakers = 3, 
            max_speakers = 3, 
            version = v21,
            globalP = True)

print('-' * 20, "RAMC GLOBAL TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = ramc_ID, 
            data_path = ramc_TEST_PATH, 
            output_path = ramc_OUTPUT_PATH_EP + PY3_1 + 'test/', 
            min_speakers = 3, 
            max_speakers = 3, 
            version = v31,
            globalP = True)
...
print('-' * 20, "RAMC BASELINE TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = ramc_ID, 
            data_path = ramc_TEST_PATH, 
            output_path = ramc_OUTPUT_PATH_BASELINE + PY2_1 + 'test/', 
            min_speakers = None,
            max_speakers = None,
            version = v21,
            globalP = False, exactNum = False)

print('-' * 20, "RAMC BASELINE TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = ramc_ID, 
            data_path = ramc_TEST_PATH, 
            output_path = ramc_OUTPUT_PATH_BASELINE + PY3_1 + 'test/', 
            min_speakers = None,
            max_speakers = None,
            version = v31,
            globalP = False, exactNum = False)
...
print('-' * 20, "RAMC EXACT NUM TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = ramc_ID, 
            data_path = ramc_TEST_PATH, 
            output_path = ramc_OUTPUT_PATH_EXACT + PY2_1 + 'test/', 
            version = v21, 
            exactNum = True, 
            gt_path = 
            ramc_TEST_LABELS_PATH)

print('-' * 20, "RAMC EXACT NUM TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = ramc_ID, 
            data_path = ramc_TEST_PATH, 
            output_path = ramc_OUTPUT_PATH_EXACT + PY3_1 + 'test/', 
            version = v31, 
            exactNum = True, 
            gt_path = ramc_TEST_LABELS_PATH)
