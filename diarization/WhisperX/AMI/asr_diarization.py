...
# =================================== AMI ===================================
# ===========================================================================
AMI_PATH = '/scratch/ssd004/scratch/amorim/datasets/AMI//headset-mix'
ami_TRAIN_PATH = os.path.join(AMI_PATH, 'train/')
ami_VAL_PATH = os.path.join(AMI_PATH, 'dev/')
ami_TEST_PATH = os.path.join(AMI_PATH, 'test/')

ami_OUTPUT_PATH_EP = os.path.join(OUTPUT_PATH, 'AMI/results/global/')
ami_OUTPUT_PATH_BASELINE = os.path.join(OUTPUT_PATH, 'AMI/results/baseline/')
ami_OUTPUT_PATH_EXACT = os.path.join(OUTPUT_PATH, 'AMI/results/exact/')

ami_TEST_LABELS_PATH = os.path.join(AMI_PATH, 'Labels/test')
...
def diarization(dataset, data_path, output_path, min_speakers = None, max_speakers = None, 
    ...
    for name in os.listdir(data_path):
        dir_path = data_path + name + '/audio/'
        
        for file in os.listdir(dir_path):
            if os.path.isfile(
                os.path.join(output_path, file.split('.')[0] + '.rttm')):
                ...
                if lang is not None:
                    ...
                    save2rttm(res["segments"], output_path, file)
...
print('-' * 20, "AMI GLOBAL TEST 2.1".center(5), '-' * 20, flush=True)
diarization(ami_ID, ami_TEST_PATH, ami_OUTPUT_PATH_EP + PY2_1 + 'test/', 3, 4, v21, True)

print('-' * 20, "AMI GLOBAL TEST 3.1".center(5), '-' * 20, flush=True)
diarization(ami_ID, ami_TEST_PATH, ami_OUTPUT_PATH_EP + PY3_1 + 'test/', 3, 4, 
            v31, True)


#---------------------------- BASELINE ----------------------------

print('-' * 20, "AMI BASELINE TEST 2.1".center(5), '-' * 20, flush=True)
diarization(ami_ID, ami_TEST_PATH, ami_OUTPUT_PATH_BASELINE + PY2_1 + 'test/', None, None, v21, False)

print('-' * 20, "AMI BASELINE TEST 3.1".center(5), '-' * 20, flush=True)
diarization(ami_ID, ami_TEST_PATH, ami_OUTPUT_PATH_BASELINE + PY3_1 + 'test/', 
            None, None, v31, False)
...
print('-' * 20, "AMI EXACT TEST 2.1".center(5), '-' * 20, flush=True)
diarization(ami_ID, ami_TEST_PATH, ami_OUTPUT_PATH_EXACT + PY2_1 + 'test/', version = v21, exactNum = True, gt_path = ami_TEST_LABELS_PATH)

print('-' * 20, "AMI EXACT TEST 3.1".center(5), '-' * 20, flush=True)
diarization(ami_ID, ami_TEST_PATH, ami_OUTPUT_PATH_EXACT + PY3_1 + 'test/', 
            version = v31, exactNum = True, gt_path = ami_TEST_LABELS_PATH)
