...
TRAIN_PATH = os.path.join(DATASET_PATH, 'Data/dev')
TEST_PATH = os.path.join(DATASET_PATH, 'Data/test')

TRAIN_LABELS_PATH = os.path.join(DATASET_PATH, 'GroundTruth/dev')
...
def create_csv(path, labels_path):
    ...
    for file in os.listdir(path):
        ...
        rows.append(["VoxConverse", round(duration/60, 3), num_speakers, round(percentage, 3), round(mean_snr, 3), file.split('.')[0]])
       
    
    # Source https://www.pythontutorial.net/python-basics/python-write-csv-file/
    with open("Vox_Statistics.csv", 'a', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
