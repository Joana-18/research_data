...
def create_csv(path, labels_path):
    ...
    for subject in os.listdir(path):
        semi_path = os.path.join(path, subject, 'audio/')
        
        for file in os.listdir(semi_path):
            file_path = os.path.join(semi_path, file)
            ...
            rows.append(["AMI", round(duration/60, 3), num_speakers, round(percentage, 3), round(mean_snr, 3), file.split('.')[0]])
       
    
    # Source https://www.pythontutorial.net/python-basics/python-write-csv-file/
    with open("AMI_Statistics.csv", 'a', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
