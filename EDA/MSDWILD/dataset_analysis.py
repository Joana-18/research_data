...
def create_csv(path, labels_path, partition):
    ...
    for file in os.listdir(path):
        ...
        if os.path.isfile(gt_file_path):
            ...
            rows.append(["MSD " + partition, round(duration/60, 3), num_speakers, round(percentage, 3), round(mean_snr, 3), file.split('.')[0]])
       
    
    # Source https://www.pythontutorial.net/python-basics/python-write-csv-file/
    with open("MSD_Statistics.csv", 'a+', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
...
create_csv(TEST_PATH, TEST_LABELS_PATH_FEW, "Few")
create_csv(TEST_PATH, TEST_LABELS_PATH_MANY, "Many")