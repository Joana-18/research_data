...
EARNINGS_DATASET_PATH = '/path/to/datasets/folder/Earnings-21/'
earnings_TEST_PATH = os.path.join(EARNINGS_DATASET_PATH, 'wav/')

earnings_TEST_LABELS_PATH = os.path.join(EARNINGS_DATASET_PATH, 'earnings-21/earnings21/rttms')

earnings_OUTPUT_PATH_EP = os.path.join(OUTPUT_PATH, 'Earnings-21/results/global/')
earnings_OUTPUT_PATH_BASELINE = os.path.join(OUTPUT_PATH, 'Earnings-21/results/baseline/')
earnings_OUTPUT_PATH_EXACT = os.path.join(OUTPUT_PATH, 'Earnings-21/results/exact/')
earnings_ID = 4
...
print('-' * 20, "EARNINGS GLOBAL TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = earnings_ID, 
            data_path = earnings_TEST_PATH, 
            output_path = earnings_OUTPUT_PATH_EP + PY2_1 + 'test/', 
            min_speakers = 2, 
            max_speakers = 20, 
            version = v21,
            globalP = True)

print('-' * 20, "EARNINGS GLOBAL TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = earnings_ID, 
            data_path = earnings_TEST_PATH, 
            output_path = earnings_OUTPUT_PATH_EP + PY3_1 + 'test/', 
            min_speakers = 2, 
            max_speakers = 20, 
            version = v31,
            globalP = True)
...
print('-' * 20, "EARNINGS BASELINE TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = earnings_ID, 
            data_path = earnings_TEST_PATH, 
            output_path = earnings_OUTPUT_PATH_BASELINE + PY2_1 + 'test/', 
            min_speakers = None,
            max_speakers = None,
            version = v21,
            globalP = False, exactNum = False)

print('-' * 20, "EARNINGS BASELINE TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = earnings_ID, 
            data_path = earnings_TEST_PATH, 
            output_path = earnings_OUTPUT_PATH_BASELINE + PY3_1 + 'test/', 
            min_speakers = None,
            max_speakers = None,
            version = v31,
            globalP = False, exactNum = False)
...
print('-' * 20, "EARNINGS EXACT NUM TEST 2.1".center(5), '-' * 20, flush=True)
diarization(dataset = earnings_ID, 
            data_path = earnings_TEST_PATH,
            output_path = earnings_OUTPUT_PATH_EXACT + PY2_1 + 'test/', 
            version = v21, 
            exactNum = True, 
            gt_path = earnings_TEST_LABELS_PATH)

print('-' * 20, "EARNINGS EXACT NUM TEST 3.1".center(5), '-' * 20, flush=True)
diarization(dataset = earnings_ID, 
            data_path = earnings_TEST_PATH, 
            output_path = earnings_OUTPUT_PATH_EXACT + PY3_1 + 'test/', 
            version = v31, 
            exactNum = True, 
            gt_path = earnings_TEST_LABELS_PATH)
